        ...
        if(index > -1)
        {
            switch (index)
            {
                ...
                case 1:
                    if(!profiles.containsKey(index))
                    {
                        new_profile = new Profile_information("Second", "Person", "(655) 123-4567", "(655)456-9999", "aliconnors@example.com", "ali_connors@example.com", R.drawable.ali_connors_img);
                        profiles.put(index, new_profile);
                    }
                    else
                        new_profile = profiles.get(index);
                ...
                case 4:
                    if(!profiles.containsKey(index))
                    {
                        new_profile = new Profile_information("Fifth", "Person", "(655) 123-4567", "(655)456-9999", "aliconnors@example.com", "ali_connors@example.com", R.drawable.ali_connors_img);
                        profiles.put(index, new_profile);
                    }
                    else
                        new_profile = profiles.get(index);
